<?php
session_start();

if(!isset($_SESSION['student_id'])){
    header("Location: ../login.php");
    exit();
}

require_once '../../config/db.php'; 
include '../navbar.php'; 
$student_id = $_SESSION['student_id'];
// SQL: Student ke enrolled courses fetch karo
$sql = "
SELECT c.id, c.title, c.description, c.image, se.progress
FROM course c
JOIN student_enrollment se ON c.id = se.course_id
WHERE se.student_id = ?
";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $student_id);
$stmt->execute();
$result = $stmt->get_result();

$courses = [];
while($row = $result->fetch_assoc()){
    $courses[] = $row;
}

// Overall progress calculate karna
$total_progress = 0;
$course_count = 0;
foreach($courses as $course){
    $total_progress += $course['progress']; // progress table ya DB se aani chahiye
    $course_count++;
}
$overall_progress = ($course_count > 0) ? round($total_progress / $course_count) : 0;

?>


<!DOCTYPE html>
<html>
<head>
    <title>Student Dashboard</title>
    <link rel="stylesheet" href="../Styles/index.css">
</head>
<body>

<div class="container">

    <div class="section">
        <h2 class="greeting">Welcome, <?php echo htmlspecialchars($_SESSION['student_name']); ?>!</h2>
        <p>Here is your dashboard overview.</p>
    </div>


     <div class="section">
<h2>My Courses</h2>

<?php if(!empty($courses)): ?>
    <?php foreach($courses as $course): ?>
        <div class="card">
            <?php echo htmlspecialchars($course['title']); ?><br>
            Progress: <?php echo $course['progress']; ?>%
            <br>
            <a href="courses.php?course_id=<?php echo $course['id']; ?>" class="button">View Details</a>
        </div>
    <?php endforeach; ?>
<?php else: ?>
    <p>You are not enrolled in any courses yet.</p>
<?php endif; ?>
</div>

    
    <div class="section">
        <h2>Analytics</h2>
        <p>Overall progress: <?php echo $overall_progress; ?>%</p>
        <a href="analytics.php" class="button">View Analytics</a>
</div>

    <div class="section">
        <h2>Certificates</h2>
        <p>View your completed course certificates.</p>
        <a href="certificates.php" class="button">View Certificates</a>
    </div>


    <div class="section">
        <h2>Achievements</h2>
        <div class="card">Badge 1</div>
        <div class="card">Badge 2</div>
        <a href="achievements.php" class="button">View Achievements</a>
    </div>

    
    <div class="section">
        <h2>My Profile</h2>
        <p><strong>Name:</strong> <?php echo htmlspecialchars($_SESSION['student_name']); ?></p>
        <p><strong>Email:</strong> <?php echo htmlspecialchars($_SESSION['student_email']); ?></p>
        <a href="profile.php" class="button">Edit Profile</a>
    </div>

</div>

</body>
</html>
